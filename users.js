...
router.post('/log', (req, res) => {
    ...
    var sql='SELECT * FROM inventorydb.users where username = \'' + username + '\' and password = \'' + password + '\'';
...
router.post("/register", (req, res) => {
    ...
    let checkRegistered = `SELECT * FROM inventorydb.managers WHERE email=?`;
    ...
    db.query(checkRegistered, email, (err, data, fields) => {
        ...
        if (data[0] === undefined) {
            ...
            let theQuery = `INSERT INTO inventorydb.managers (firstname,lastname, phone, email, gender) VALUES(?, ?, ?, ?, ?)`;
            ...
            db.query(theQuery, values, (err, data) => {
                if (err) {
                    ...
                    let theUserQuery = `INSERT INTO inventorydb.users (username, password) VALUES(?, ?)`
