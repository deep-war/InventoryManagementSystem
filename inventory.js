...
router.post('/add', function(req, res, next) {
      ...
      var sql = `INSERT INTO inventorydb.itemdetails (itemName, quantity, pricePerItem, categoryName) VALUES ('${itemname}', ${quantity}, ${price}, '${category}')`;
...
router.get('/item-list', function(req, res, next) {
  var sql='SELECT * FROM inventorydb.itemdetails where quantity <> 0 order by categoryName, itemName';
...
router.post('/delete',function(req, res) {
      ...
      console.log(name);
      let sql =`DELETE FROM inventorydb.itemdetails WHERE itemName= ?`;
...
router.post('/update',function(req, res) {
    ...
    let sql2 =`UPDATE inventorydb.itemdetails SET pricePerItem=? WHERE itemName=?`;
  ...
  router.get('/price-hike-view', function(req, res, next) {
    var sql='SELECT * FROM inventorydb.priceHike';
    db.query(sql, function (err, data, fields) {
    if (err){
      console.log(err);
      throw err;
